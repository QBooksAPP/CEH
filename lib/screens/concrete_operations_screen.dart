...
class ConcreteOperationsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Concrete Operations', style: TextStyle(fontWeight: FontWeight.w900))),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          _tile(context, 'Calibration Field Sheet', 'Enter mixer calibration trials', Icons.fact_check_outlined, () {
            Navigator.push(context, MaterialPageRoute(builder: (_) => CalibrationFieldSheetScreen(session: session)));
          }),
  ...
  void _placeholder(BuildContext context, String title) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => ModulePlaceholderScreen(
      title: title,
      message: '$title will be connected after the Calibration Field Sheet is confirmed.',
    )));
  }
}
