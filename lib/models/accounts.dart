...
class BillingInvoiceLine {
  const BillingInvoiceLine({required this.lineNo,required this.description,required this.sourceType,required this.revenueAccountId,required this.net,required this.vat,required this.total,this.quantity,this.unitName,this.unitPrice,this.projectId,this.project,this.equipmentId,this.equipment,this.productionAllocations=const []});
  final int lineNo; final String description; final String sourceType; final int revenueAccountId; final double net; final double vat; final double total; final double? quantity; final String? unitName; final double? unitPrice; final int? projectId; final String? project; final int? equipmentId; final String? equipment; final List<Map<String,dynamic>> productionAllocations;
  factory BillingInvoiceLine.fromJson(Map<String,dynamic> json)=>BillingInvoiceLine(lineNo:_int(json['line_no']),description:'${json['description']??''}',sourceType:'${json['source_type']??''}',revenueAccountId:_int(json['revenue_account_id']),net:_double(json['net_amount']),vat:_double(json['vat_amount']),total:_double(json['gross_amount']),quantity:json['quantity']==null?null:_double(json['quantity']),unitName:json['unit_name']?.toString(),unitPrice:json['unit_price']==null?null:_double(json['unit_price']),projectId:json['project_id']==null?null:_int(json['project_id']),project:json['project_snapshot']?.toString(),equipmentId:json['mixer_id']==null?null:_int(json['mixer_id']),equipment:json['mixer_snapshot']?.toString(),productionAllocations:(json['production_allocations'] as List? ?? const []).map((e)=>Map<String,dynamic>.from(e as Map)).toList());
}

class BillingInvoiceDetail {
  const BillingInvoiceDetail({required this.id,required this.reference,required this.clientId,required this.client,required this.status,required this.invoiceDate,required this.paymentTerm,required this.terms,required this.vatMode,required this.vatTaxCodeId,required this.vatRate,required this.net,required this.vat,required this.total,required this.amountPaid,required this.whtAllocated,required this.outstanding,required this.lines,required this.creditNotes,this.issuedAt,this.journalId,this.postingStatus});
  final int id; final String reference; final int clientId; final String client; final String status; final String invoiceDate; final String paymentTerm; final String terms; final String vatMode; final int? vatTaxCodeId; final String? vatRate; final double net; final double vat; final double total; final double amountPaid; final double whtAllocated; final double outstanding; final List<BillingInvoiceLine> lines; final List<Map<String,dynamic>> creditNotes; final String? issuedAt; final int? journalId; final String? postingStatus;
  factory BillingInvoiceDetail.fromJson(Map<String,dynamic> data){final json=Map<String,dynamic>.from(data['invoice'] as Map);return BillingInvoiceDetail(id:_int(json['id']),reference:'${json['reference']??''}',clientId:_int(json['client_id']),client:'${json['client_name_snapshot']??''}',status:'${json['display_status']??json['status']??''}',invoiceDate:'${json['invoice_date']??''}',paymentTerm:'${json['payment_term']??''}',terms:'${json['terms_snapshot']??json['payment_term']??''}',vatMode:'${json['vat_mode']??'NONE'}',vatTaxCodeId:json['vat_tax_code_id']==null?null:_int(json['vat_tax_code_id']),vatRate:json['vat_rate_snapshot']?.toString(),net:_double(json['net_amount']),vat:_double(json['vat_amount']),total:_double(json['total_amount']),amountPaid:_double(json['amount_paid']),whtAllocated:_double(json['wht_allocated']),outstanding:_double(json['outstanding']),lines:(data['lines'] as List? ?? const []).map((e)=>BillingInvoiceLine.fromJson(Map<String,dynamic>.from(e as Map))).toList(),creditNotes:(data['credit_notes'] as List? ?? const []).map((e)=>Map<String,dynamic>.from(e as Map)).toList(),issuedAt:json['issued_at']?.toString(),journalId:json['journal_id']==null?null:_int(json['journal_id']),postingStatus:json['posting_status']?.toString());}
}
