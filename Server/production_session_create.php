...
$stmt = $db->prepare("INSERT INTO qbook_production_sessions (production_date, client_id, client_name, project_site, mixer_id, mixer_code_snapshot, mixer_name_snapshot, loading_point, discharge_point, operator_id, operator_name_snapshot, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->execute($values);
$id = (int)$db->lastInsertId();
qbook_audit($user, 'PRODUCTION_SESSION_CREATED', 'PRODUCTION_SESSION', $id, ['client_id' => $clientId, 'mixer_id' => $mixerId]);
