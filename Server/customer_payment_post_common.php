...
function customer_payment_post(PDO $db, array $user, array $input): array {
    ...
    return bank_payment_transaction($db, (int)($input['receipt_id'] ?? 0), function () use ($db, $user, $input): array {
        ...
        foreach ($allocations as $allocation) if (!is_array($allocation)) accounts_fail('INVALID_ALLOCATION');
        usort($allocations, static fn(array $a,array $b):int => (int)($a['invoice_id']??0) <=> (int)($b['invoice_id']??0));
        $cashAllocated = 0;
        $whtAllocated = 0;
        $normalized = [];
        $invoiceIds = [];
        ...
        if ($backed) {
            ...
            $db->prepare("UPDATE qbook_bank_statement_rows SET status='RECONCILED',potential_source_type='CUSTOMER_RECEIPT',potential_source_id=? WHERE id=?")
                ->execute([$id, $receipt['statement_row_id']]);
            accounts_audit($db, $user, 'BANK_ROW_RECONCILED', 'BANK_MATCH', $matchId,
                ['statement_row_id'=>(int)$receipt['statement_row_id'],'source_type'=>'CUSTOMER_RECEIPT','source_record_id'=>$id,'journal_posted'=>false,'method'=>'PAYMENT_POSTING']);
        }
        return [
            'receipt' => [
