...
accounts_endpoint(function() use($user,$input): array {
    ...
    return accounts_transaction($db,function() use($db,$user,$id): array {
        ...
        if((int)$e['line_model_version']===1){$lines=accounts_load_expense_lines($db,'qbook_petty_cash_expense_lines',$id,true);$lineMinor=0;foreach($lines as $line){$lineMinor+=accounts_money_minor($line['amount'],false);accounts_expense_lines($db,[['amount'=>$line['amount'],'expense_account_id'=>$line['expense_account_id'],'description'=>$line['item_description'],'quantity'=>$line['quantity'],'unit_price'=>$line['unit_price'],'client_id'=>$line['client_id'],'project_id'=>$line['project_id'],'mixer_id'=>$line['mixer_id']]]);}if($lines===[]||$lineMinor!==$amount) accounts_fail('EXPENSE_LINE_TOTAL_MISMATCH');}
