...
accounts_endpoint(function() use($user,$input): array {
    ...
    return accounts_transaction($db,function() use($db,$user,$id,$action,$reason): array {
        ...
        if($action==='CORRECTION_REQUIRED'){
            ...
            $minor=accounts_money_minor($e['amount'],false);$petty=accounts_account_id($db,'1200');$journalLines=[];
            if((int)$e['line_model_version']===1){$lines=accounts_load_expense_lines($db,'qbook_petty_cash_expense_lines',$id,true);$sum=0;foreach($lines as $line){$lineMinor=accounts_money_minor($line['amount'],false);$sum+=$lineMinor;$journalLines[]=['account_id'=>(int)$line['expense_account_id'],'debit_minor'=>$lineMinor,'credit_minor'=>0,'cost_centre_id'=>$line['cost_centre_id'],'client_id'=>$line['client_id'],'project_id'=>$line['project_id'],'mixer_id'=>$line['mixer_id'],'description'=>$line['item_description']];}if($lines===[]||$sum!==$minor) accounts_fail('EXPENSE_LINE_TOTAL_MISMATCH');}
            else{$journalLines[]=['account_id'=>(int)$e['expense_account_id'],'debit_minor'=>$minor,'credit_minor'=>0,'client_id'=>$e['client_id'],'project_id'=>$e['project_id'],'mixer_id'=>$e['mixer_id'],'description'=>$e['description']];}
            $journalLines[]=['account_id'=>$petty,'debit_minor'=>0,'credit_minor'=>$minor,'custodian_user_id'=>(int)$e['custodian_user_id'],'description'=>$e['description']];
            $journal=accounts_post_journal($db,$user,['transaction_date'=>$e['expense_date'],'description'=>'Petty cash expense: '.$e['description'],'source_module'=>'PETTY_CASH_EXPENSE','source_record_id'=>$id,'approved_by'=>(int)$user['id']],$journalLines);
