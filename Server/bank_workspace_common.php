...
function bank_workspace_usage_sql():string {
 ...
 WHEN EXISTS(SELECT 1 FROM qbook_customer_receipts c WHERE c.statement_row_id=r.id AND c.status='DRAFT') THEN 'RESERVED_PAYMENT'
...
function bank_workspace_owner(PDO $db,int $row):?array {
 ...
 $q=$db->prepare("SELECT id,status FROM qbook_customer_receipts WHERE statement_row_id=? AND NOT(status='CANCELLED' AND journal_id IS NULL)");$q->execute([$row]);if($v=$q->fetch())return ['type'=>'CUSTOMER_RECEIPT','id'=>(int)$v['id'],'relationship'=>$v['status']==='DRAFT'?'Reserved for Client Payment':'Client Payment','status'=>$v['status']];
...
function bank_workspace_register(PDO $db,array $in):array {
 ...
 $usage=bank_workspace_usage_sql();$state=$in['usage']??'ALL';if(!in_array($state,['ALL','AVAILABLE','RESERVED_EXPENSE','RESERVED_PAYMENT','EXPENSE','REFUND','PAYMENT','RECONCILED'],true))accounts_fail('INVALID_USAGE');if($state!=='ALL'){$where[]="($usage)=?";$args[]=$state;}
