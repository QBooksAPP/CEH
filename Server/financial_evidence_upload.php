...
accounts_endpoint(function() use($user,$input): array {
    $type=strtoupper(trim((string)($input['source_type']??'')));$sourceId=(int)($input['source_record_id']??0);if($sourceId<=0||!in_array($type,['PETTY_CASH_FUNDING','PETTY_CASH_EXPENSE'],true)) accounts_fail('INVALID_EVIDENCE_SOURCE');
