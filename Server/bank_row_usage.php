...
function bank_row_available(PDO $db,int $id,string $ownType='',int $ownId=0):array {
 ...
 $s=$db->prepare("SELECT id FROM qbook_general_expenses WHERE created_from_statement_row_id=? AND status NOT IN('CANCELLED_NOT_SPENT','VOIDED') FOR UPDATE");$s->execute([$id]);
