...
function bank_transaction_detail_read(PDO $db,array $user,array $input):array {
 ...
 $q=$db->prepare("SELECT a.event_type action,a.created_at timestamp,u.full_name actor,a.source_type,a.source_record_id,a.details_json FROM qbook_financial_audit a LEFT JOIN qbook_users u ON u.id=a.actor_user_id WHERE (a.source_type='GENERAL_EXPENSE' AND a.source_record_id IN (SELECT e.id FROM qbook_general_expenses e WHERE e.created_from_statement_row_id=? UNION SELECT f.expense_id FROM qbook_general_expense_refunds f WHERE f.statement_row_id=?)) OR (a.source_type='BANK_MATCH' AND a.source_record_id IN (SELECT m.id FROM qbook_bank_matches m WHERE m.statement_row_id=?)) OR (a.source_type='CUSTOMER_RECEIPT' AND a.source_record_id IN (SELECT p.id FROM qbook_customer_receipts p WHERE p.statement_row_id=?)) ORDER BY a.id DESC LIMIT 100");$q->execute([$id,$id,$id,$id]);$history=$q->fetchAll();
 foreach($history as &$event){
  ...
  elseif($type==='CUSTOMER_RECEIPT'){$lookup=$db->prepare('SELECT reference_no FROM qbook_customer_receipts WHERE id=?');$lookup->execute([$sourceId]);$ref=$lookup->fetchColumn();if($ref!==false)$event['source_reference']=billing_ref('RECEIPT',$ref);}
  if(($details['method']??null)==='PAYMENT_POSTING')$event['method']='PAYMENT_POSTING';
  unset($event['details_json'],$event['source_record_id']);
 }unset($event);
 return ['bank'=>$bank,'transaction'=>$row,'history'=>$history,'can_create_expense'=>$owner===null&&$row['usage_state']==='AVAILABLE'&&accounts_money_minor($row['amount'],false)<0,'can_match'=>$owner===null&&$row['usage_state']==='AVAILABLE'&&accounts_money_minor($row['amount'],false)<0,'can_link_refund'=>$owner===null&&$row['usage_state']==='AVAILABLE'&&accounts_money_minor($row['amount'],false)>0,'can_record_payment'=>$owner===null&&$row['usage_state']==='AVAILABLE'&&accounts_money_minor($row['amount'],false)>0];
 });
}
