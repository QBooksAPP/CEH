...
void main() {
  ...
  test('server file pipeline does not post journals or accept supplied rows', () {
    ...
    expect(source('bank_statement_import.php'), isNot(contains("\$input['rows']")));
  });
  test('migration replaces content uniqueness with physical-row uniqueness', () {
    ...
    expect(migration, contains('uq_bank_physical_row(import_batch_id,source_sheet,source_row)'));
  ...
  test('existing row consumers share authoritative ownership guard', () {
    for (final name in [
      ...
      expect(source(name), contains('bank_row_usage.php'));
    }
