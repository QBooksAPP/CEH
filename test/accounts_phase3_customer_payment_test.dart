...
void main() {
  group('customer payment accounting', () {
    ...
    test('later application uses only calculated customer credit', () {
      expect(apply, contains('SUM(cash_amount)'));
      expect(
          apply, contains(r'$availableMinor = accounts_money_minor($receipt'));
      expect(apply, contains(r'- $allocatedMinor - $usedMinor'));
      expect(apply, contains('CUSTOMER_CREDIT_INTEGRITY_ERROR'));
      expect(
          apply, contains("billing_account_role(\$db,'CUSTOMER_ADVANCES')"));
      expect(
          apply, contains("billing_account_role(\$db,'TRADE_RECEIVABLES')"));
    });
  ...
  testWidgets('business-facing payment supports multiple invoice allocations',
    ...
    await tester.pumpAndSettle();
    await tester
        .tap(find.byKey(const ValueKey('save-post-customer-payment')));
    await tester.pumpAndSettle();
